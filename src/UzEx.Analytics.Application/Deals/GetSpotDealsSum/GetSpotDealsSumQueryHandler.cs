...
public sealed class GetSpotDealsSumQueryHandler : IQueryHandler<GetSpotDealsSumQuery, GetSpotDealsSumResponse>
{
    private readonly IApplicationDbContext _dbContext;

    public GetSpotDealsSumQueryHandler(IApplicationDbContext dbContext)
    {
        _dbContext = dbContext;
    }
    
    public async Task<Result<GetSpotDealsSumResponse>> Handle(GetSpotDealsSumQuery request, CancellationToken cancellationToken)
    {
        var amountSum = await _dbContext
            .Deals
            .Where(deal => deal.Contract != null && deal.Contract.Form == ContractForm.Spot)
            .AsNoTracking()
        ...
        var response = new GetSpotDealsSumResponse
        {
            Sum = sum,
            Unit = unit,
        };
