...
public sealed class GetRevenueByContractFormOverTimeQueryHandler
    : IQueryHandler<GetRevenueByContractFormOverTimeQuery, List<GetRevenueByContractFormOverTimeResponse>>
{
    private readonly IApplicationDbContext _dbContext;

    public GetRevenueByContractFormOverTimeQueryHandler(IApplicationDbContext dbContext)
    {
        _dbContext = dbContext;
    }

    public async Task<Result<List<GetRevenueByContractFormOverTimeResponse>>> Handle(GetRevenueByContractFormOverTimeQuery request, CancellationToken cancellationToken)
    {
        ...
        var result = aggregatedData
            ...
            .Select(g => new GetRevenueByContractFormOverTimeResponse
            {
                ContractForm = Enum.GetName(typeof(ContractForm), g.Key.ContractForm)!,
                Year = g.Key.Year,
                Data = g.OrderBy(x => x.Month)
                .Select(x => new RevenueOfContractFormByMonthDataItem
                {
                    MonthIndex = x.Month,
