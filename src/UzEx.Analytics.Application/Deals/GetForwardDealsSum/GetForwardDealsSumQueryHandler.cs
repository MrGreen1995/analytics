...
public class GetForwardDealsSumQueryHandler : IQueryHandler<GetForwardDealsSumQuery, GetForwardDealsSumResponse>
{
    private readonly IApplicationDbContext _dbContext;

    public GetForwardDealsSumQueryHandler(IApplicationDbContext dbContext)
    {
        _dbContext = dbContext;
    }

    public async Task<Result<GetForwardDealsSumResponse>> Handle(GetForwardDealsSumQuery request, CancellationToken cancellationToken)
    {
        var amountSum = await _dbContext
            .Deals
            .Where(deal => deal.Contract != null && deal.Contract.Form == ContractForm.Forward)
            .AsNoTracking()
        ...
        var response = new GetForwardDealsSumResponse()
        {
            Sum = sum,
            Unit = unit,
        };
