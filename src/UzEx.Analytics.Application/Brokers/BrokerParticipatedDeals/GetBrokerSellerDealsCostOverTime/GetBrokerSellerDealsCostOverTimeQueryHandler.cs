...
public sealed class GetBrokerSellerDealsCostOverTimeQueryHandler
    : IQueryHandler<GetBrokerSellerDealsCostOverTimeQuery, List<GetBrokerHandledDealsCostOverTimeResponse>>
{
    private readonly IApplicationDbContext _dbContext;

    public GetBrokerSellerDealsCostOverTimeQueryHandler(IApplicationDbContext dbContext)
    {
        _dbContext = dbContext;
    }

    public async Task<Result<List<GetBrokerHandledDealsCostOverTimeResponse>>> Handle(GetBrokerSellerDealsCostOverTimeQuery request, CancellationToken cancellationToken)
    {
        ...
        var aggregatedData = await _dbContext.Deals
            .Where(d =>
            d.SellerBrokerId == request.Request.Id &&
        ...
        var mergedData = allMonths
            ...
            .Select(yearGroup => new GetBrokerHandledDealsCostOverTimeResponse
            {
                ...
                ClientDirection = "Seller Broker Deals",
                Year = yearGroup.Key,
                Data = yearGroup.Select(m =>
                {
